...
public class DefaultNetTaskActor extends NetActor
{
	...
	public boolean processFusionMessage( final FusionMessage msg )
	{
		...
		if (!(msg instanceof NetTaskMessage))
		{
			...
			msg.publishMessageError(FusionMessage.ERROR_CODE_SYS_ERROR, FusionMessage.ERROR_MSG_SYS_ERROR, FusionMessage.ERROR_MSG_SYS_ERROR);
			return true;
		}
	...
	private HttpPost createCommonHttpRequest( NetTaskMessage netTaskMessage )
	{
		...
		if (netTaskMessage != null)
		{
			...
			HashMap<String, String> additionalHeaders = netTaskMessage.getHeaders();
			for (String propertyName : additionalHeaders.keySet())
			{
				httpRequest.addHeader(propertyName, (String)additionalHeaders.get(propertyName));
			}
	...
	private Object syncPaser( NetTaskMessage netTaskMessage, HttpResponse response, String charSetName )
	{
		...
		if (response != null)
		{// 返回状态码不是200 //TODO:网络错误还不能检测
			
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK)
			{
				netTaskMessage.publishMessageError(errorCode, errorMsg, errorDescription);
			} else
			{
				try
				{
		...
		if (hasException)
		{
			...
			{
				netTaskMessage.setErrorWithoutNotify(errorCode, errorMsg, errorDescription);
